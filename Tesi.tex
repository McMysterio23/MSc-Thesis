%\documentclass[twoside,single]{lion-msc}
\documentclass[oneside,single]{lion-msc} %We want a good pdf, not a good printed thesis !


\title{{ \texttt{Picosecond Jitter of Picosecond Pulses}}}
\author{Andrea Maccarinelli}
\studentid{4535286}                           % check you student ID, LaTeX does not do this
\abstract{Optical quantum technologies and quantum optics experiments often require picosecond-scale optical pulses, and knowledge of their timing properties is crucial. This thesis focuses on analyzing the timing jitter introduced by the electronic circuit driving the modulators using superconducting single-photon detectors and photon correlation measurements. As a reference and to characterize the measurement setup and detector response, low-jitter femto-second scale optical pulses from a mode-locked laser are used. The study introduces a novel method to identify the central peak in the photon correlations. Additionally, it highlights the importance of properly setting electronic thresholds in the detection system to reduce noise and erroneous multiple detections. The results are a starting point to assess in detail the temporal jitter of pico-second optical pulses widely used in modern quantum optics experiments.}     % limit your self to 1/2 page or 500 words
\supervisor{Dr. Wolfgang Löffler}                         % Note that this should be a LION staff member!
\corrector{Dr. Evert van Nieuwenburg}                   % This could be a LION staff member or your external supervisor

\degree{Master of Science}                     % The default option is "Bachelor of Science", change if needed

%\major{Physics and Astronomy}                  % The default option is "Physics", change if needed
%\major{Physics and Mathematics}

% optional cover picture - should be jpg or pdf
% \coverpicture{\includegraphics[width=13cm]{thesisstyle.png}}

% Use this to make hyperlinks visible in the document.
% \hypersetup{colorlinks=true}

% ---------------------------------------------------------------- My defintions!
% \renewcommand{\vec}[1] {\ensuremath{ \overrightarrow{ #1 } }}
\renewcommand{\vec}[1] {\ensuremath{ \mathbf{ #1 } }}
% \bra \ket \braket and \proj
\newcommand{\bra}[1]{\ensuremath{\langle #1 \vert}}
\newcommand{\ket}[1]{\ensuremath{\vert #1 \rangle}}
\newcommand{\braket}[2]{\ensuremath{\langle #1 \vert #2 \rangle}}
\newcommand{\proj}[1]{\ensuremath{\vert #1 \rangle \langle #1 \vert}}

\newcommand{\kpar}{\ensuremath{k_\parallel}}
% ----------------------------------------------------------------

% \usepackage{tocloft}
% \renewcommand{\cftchapdotsep}{\cftdotsep}
\usepackage{comment}


% Redefine autoref names to be capitalized
\AtBeginDocument{%
  \renewcommand{\sectionautorefname}{Section}
  \renewcommand{\subsectionautorefname}{Subsection}
  \renewcommand{\chapterautorefname}{Chapter}
  \renewcommand{\figureautorefname}{Figure}
  \renewcommand{\tableautorefname}{Table}
  \renewcommand{\equationautorefname}{Equation}
}

% Optional: enable visible colored links (if hidelinks is unwanted)
% \hypersetup{
%   hidelinks=false,
%   colorlinks=true,
%   linkcolor=blue,
%   urlcolor=blue,
%   citecolor=blue
% }
\usepackage[table]{xcolor}
\usepackage{colortbl}
\usepackage{array}
\usepackage{caption}
\sloppy

\begin{document}

% roman numbering in the table of contents section
\pagenumbering{roman}

\maketitle

% Table of contents :  it is a good idea to include this into your thesis
\tableofcontents
\cleardoublepage

% The following list of figures and list of tables are optional. Remove the comments if needed
%\listoffigures
%\newpage

%\listoftables
%\newpage

% in the main part of the document use standard arabic numbers. Page counter resets to 1.
\pagenumbering{arabic}


%\maketitle

\include{Intro_Chapter}
\include{Chapter2}
\include{Chapter3}
\include{Chapter4}
\include{Chapter5}
\include{Ringraziamenti}


\bibliographystyle{plain}
\bibliography{bibliography}
\end{document}

